...
export function BalloonsAnimation({ gender, revealed, onComplete }: BalloonsAnimationProps) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const color = colorRand > 0.6 ? mainColor : (colorRand > 0.3 ? lightColor : thirdColor);
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      >
        <motion.div 
          className={`text-7xl sm:text-9xl font-bold ${gender === 'boy' ? 'text-baby-blue-dark' : 'text-baby-pink-dark'}`}
