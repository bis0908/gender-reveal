...
import { DDayGeneratedLinkCard } from "@/components/reveal-form/dday-generated-link-card";
import { DDayOptionForm } from "@/components/reveal-form/dday-option-form";
...
import { type FormValues, formSchema } from "@/lib/schemas/reveal-form-schema";

interface RevealFormProps {
  onLinkGenerated?: () => void;
}

export function RevealForm({ onLinkGenerated }: RevealFormProps = {}) {
  const { toast } = useToast();
  const { t } = useTranslation();
  const [loading, setLoading] = useState(false);
  const [generatedLink, setGeneratedLink] = useState<string | null>(null);
  const [ddayLinks, setDdayLinks] = useState<{
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      // D-Day 예약 여부 확인
      const isDDayReservation = !!data.scheduledAt;
      ...
      if (isDDayReservation) {
        // D-Day 예약 API 호출
        const response = await fetch("/api/dday/create", {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          throw new Error(
            responseData.error?.message || "D-Day 예약 생성에 실패했습니다.",
          );
        }
        ...
        const revealLink = `${window.location.origin}/reveal?token=${encodeURIComponent(revealToken)}`;
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        >
          ...
          >
            ...
            {/* D-Day 옵션 */}
            <DDayOptionForm form={form} />

            {/* 생성된 링크 표시 */}
            {ddayLinks && (
              <DDayGeneratedLinkCard
                countdownLink={ddayLinks.countdownLink}
                revealLink={ddayLinks.revealLink}
                scheduledAt={ddayLinks.scheduledAt}
              />
            )}

            {generatedLink && !ddayLinks && (
