...
export function RevealResults({
	...
	const getBabiesGenderSummary = () => {
		...
		const girlsCount = babiesInfo.filter(
			(baby) => baby.gender === "girl",
		).length;
	...
	return (
		<div
			...
			className="container mx-auto py-10 px-4 max-w-4xl result-section"
		>
							...
							{babiesInfo.map((baby, index) => (
								<motion.div
									...
									className={`p-4 rounded-lg shadow-sm border-2 ${
										baby.gender === "boy"
											? "border-baby-blue-light bg-baby-blue-light/10"
											: "border-baby-pink-light bg-baby-pink-light/10"
									}`}
								>
									<h3
