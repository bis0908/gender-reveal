...
function RevealContent() {
  ...
  const goToNextBaby = () => {
    if (isMultipleBabies && revealData?.babiesInfo) {
      if (currentBabyIndex < revealData.babiesInfo.length - 1) {
        ...
        // startCountdown은 유지, currentBabyIndex 변경으로 컴포넌트 재마운트
      } else {
        setIsFinished(true);
      }
    } else {
  ...
  const { motherName, fatherName, animationType, countdownTime } = revealData;
